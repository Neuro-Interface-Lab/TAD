...
#### Usefull Functions
def on_delta_t(digital_recording, triggers, fsample, delta_t):
    """
    A function to detect triggers one rising edges and create a event of delta_t

    """
    for i in range(1, len(digital_recording)):
        if digital_recording[i] > digital_recording[i-1]:
            triggers.add_interval_slot(start = round(i)/fsample, end = round((round(i)/fsample + round(delta_t*fsample)/fsample)*fsample)/fsample, ID = "stim_ON", blank = True)

def on_off_interpretor(digital_recording, triggers, fsample):
    """
    This is a sketch of the function that will be used for taking odd rising as start of time slot and even rising as end of time slot
    """
    rising_edges = []
    for i in range(1, len(digital_recording)):
        if digital_recording[i] > digital_recording[i-1]:
    ...
    for i in range(1, len(rising_edges), 2):
       triggers.add_interval_slot(start=rising_edges[i-1]/fsample, end=rising_edges[i]/fsample, ID = "stim_ON", blank = False)
...
class MCSData:
    ...
    def plot_traces(self, tstart=None, tstop=None, channel_ids=None, show=True):
        """
